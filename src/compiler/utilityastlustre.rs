...
pub fn type_expr(expr: &Expr) -> Typ {
    match expr {
        Expr::Econst(c) | Expr::Efby(c, _) => type_cst(c),
        Expr::Evar(v) => v.vtype.clone(),
        Expr::Ebinop(_, e, _) | Expr::Eunop(_, e) | Expr::Emerge(_, e, _) | Expr::Ewhen(e, _) => {
            type_expr(e)
        }
...
pub fn gather_vars_aux(expr: &Expr, mut vars: Vec<Var>) -> Vec<Var> {
    match expr {
        ...
        Expr::Ebinop(_, e1, e2) | Expr::Emerge(_, e1, e2) => {
            let vars = gather_vars_aux(e1, vars);
            gather_vars_aux(e2, vars)
        }
        Expr::Eunop(_, e) | Expr::Efby(_, e) | Expr::Ewhen(e, _) => gather_vars_aux(e, vars),
        Expr::Ecall(_, vs) => {
...
pub fn outside_of_pre(expr: &Expr, mut vars: Vec<Var>) -> Vec<Var> {
    match expr {
        Expr::Econst(_) => vars,
        Expr::Evar(v) => {
...
pub fn translate_var(var: &Var, role: CVarRole, init_value: Option<&Constant>) -> CVar {
    astc::build_cvar(
        ...
        0,
        role,
        translate_const_aux(init_value),
    )
}
...
pub fn build_base_cprog(node: &Node) -> CProg {
    ...
    for var in &node.output {
        prog.outputs
            .push(translate_var(var, CVarRole::Output, None))
    }
    for var in &node.local_vars {
        prog.local_vars
            .push(translate_var(var, CVarRole::LocalVar, None))
    }
    prog
}
