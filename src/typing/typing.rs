...
pub fn type_decl(
    obj: &FileObj,
    type_tbl: &mut HashMap<String, ASTTypeT>,
    var_tbl: &mut HashMap<String, ASTTypeT>,
    d: ASTOneDeclT,
) -> ASTOneDeclT {
    match d {
        ASTOneDeclT::ASTConstDecl(vec) => {
            ...
            for v in vec {
                match v.val {
                    ...
                    None => {
                        // no default set of constants
                        obj.print_error_message(
                            v.name,
                            "This constant does not have any values preset",
                        );
                        panic!()
                    }
                }
...
fn fill_type_in_expr(
    obj: &FileObj,
    type_tbl: &mut HashMap<String, ASTTypeT>,
    label_tbl: &mut HashMap<String, ASTTypeT>,
    expr: ASTExprT,
) -> ASTExprT {
    match expr {
        ASTExprT::ASTConst(span, asttype_t, astconst_t) => match astconst_t {
            ASTConstT::ASTBool(_) => ASTExprT::ASTConst(span, ASTTypeT::ASTBool, astconst_t),
        ...
        ASTExprT::ASTUnop(span, _, astunop_t, astexpr_t) => match astunop_t {
            ASTUnopT::ASTNot => {
                ...
                if *tt == ASTTypeT::ASTBool {
                    ...
                    obj.print_error_message(
                        span,
                        format!("Typing error : not support only booleans, not {:?}", tt).as_str(),
                    );
                    panic!()
                }
            }
            ASTUnopT::ASTMinus => {
                ...
                if *tt == ASTTypeT::ASTInt {
                    ...
                    obj.print_error_message(
                        span,
                        format!(
                            "Typing error : not support only integers and reals, not {:?}",
                            tt
                        )
                        .as_str(),
                    );
            ...
            ASTUnopT::ASTPre | ASTUnopT::ASTCurrent => {
                let expr = fill_type_in_expr(obj, type_tbl, label_tbl, *astexpr_t);
                let tt = get_type(&expr);
        ...
        ASTExprT::ASTBinop(span, _, astbinop_t, astexpr_t1, astexpr_t2) => match astbinop_t {
            ASTBinopT::ASTWhen => {
            ...
            ASTBinopT::ASTFby | ASTBinopT::ASTArrow => {
                let expr1 = fill_type_in_expr(obj, type_tbl, label_tbl, *astexpr_t1);
                let expr2 = fill_type_in_expr(obj, type_tbl, label_tbl, *astexpr_t2);
                let tt1 = get_type(&expr1);
                let tt2 = get_type(&expr2);
            ...
            ASTBinopT::ASTAnd | ASTBinopT::ASTOr | ASTBinopT::ASTXor | ASTBinopT::ASTImpl => {
                let expr1 = fill_type_in_expr(obj, type_tbl, label_tbl, *astexpr_t1);
                let expr2 = fill_type_in_expr(obj, type_tbl, label_tbl, *astexpr_t2);
                let tt1 = get_type(&expr1);
                let tt2 = get_type(&expr2);
...
fn fill_type(obj: &FileObj, type_tbl: &mut HashMap<String, ASTTypeT>, t: ASTTypeT) -> ASTTypeT {
    match t {
        ASTTypeT::ASTNone | ASTTypeT::ASTBool | ASTTypeT::ASTInt | ASTTypeT::ASTReal => t,
        ASTTypeT::ASTLabel(l) => match type_tbl.get(&obj.span_str(l)) {
            Some(tt) => tt.clone(),
            None => {
